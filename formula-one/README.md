...
formula-one is an ergonomic React Form library for building high preformance and expressive forms.
...
Our form should allow users to add pets to their list of pets, and remove a pet if they so desire. formula-one handles this.
...
Imagine you're working on a complex form where you must be strict about data entry formatting. There are form level errors and section level errors and any error will bar the form from being submitted. This is what formula-one was built for!
...
In the above example, the FLErrorBox component displays all nested validation errors. formula-one abstracts away the complexity of validation handling and allows you to  aggregate and display all nested validation errors from a given form node.
